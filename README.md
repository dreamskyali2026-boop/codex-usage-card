...
![Codex 用量卡片高清预览（账号信息已脱敏）](docs/card-preview.png)

上图基于本机实时读取的额度数据制作，账号信息已脱敏；额度和倒计时会随刷新而变化。
...
- 菜单栏用量标记常驻显示主窗口用量：左键直接显示或隐藏卡片，右键打开选项菜单（显示卡片 / 刷新 / 退出）
...
- 拖动卡片任意空白处可移动，位置会被记住；点「−」收起时，卡片会沿曲线被拉向顶部图标，由圆角胶囊接住内容、轻微回弹后收口到菜单栏
...
如需确认完整编译没有任何 Swift 警告，可运行：

```bash
bash Tests/test_warning_free_build.sh
```
...
- **收起到顶部菜单栏**：卡片会模拟灵动岛的吸入感：先沿曲线被拉向图标、再由圆角胶囊接住并轻微回弹，最后收口到真实的顶部菜单栏用量图标；左键点顶部图标即可恢复
- **关闭**：隐藏卡片；左键点菜单栏图标即可唤回
- **顶部菜单栏图标**：左键在“显示卡片”和“隐藏卡片”之间切换；右键保留“显示／隐藏卡片、立即刷新、退出”菜单
- **设置**（同面板内切换，点 `✕` 返回）：
...
Sources/CodexUsageCard/
  ...
  Icon.swift                      应用图标离屏绘制
  MinimizeAnimationGeometry.swift 最小化吸入动画的菜单栏终点计算
build.sh / make_icon.sh           打包脚本
Assets/AppIcon.icns               生成的图标
docs/card-preview.png             README 展示图（实时界面，已脱敏）
Tests/test_minimize_animation_geometry.sh  吸入动画终点回归检查
Tests/test_warning_free_build.sh           独立目录的无警告完整构建检查
```
